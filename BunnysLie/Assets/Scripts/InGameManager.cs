...
public class InGameManager : MonoBehaviour
{
    ...
    internal void ShowLoserOfThisRound(int loserId, byte count, int round)
    {
        foreach (var rp in RemotePlayerUIDrawers)
        {
            ...
            {
                LocalPlayerUIDrawer.ShowPanelOnScreenCenter(round + "라운드 꼴지 : " + loserId + "번 플레이어", rp.Character + 2);
                rp.SetOutCount(count);
            }
        }
        if (LocalPlayer.ID == loserId)
        {
            LocalPlayerUIDrawer.ShowPanelOnScreenCenter(round + "라운드 꼴지 : " + loserId + "번 플레이어", LocalPlayerUIDrawer.Character + 2);
    ...
    {
        //로컬의 것들은 그냥 카드를 직접 넘겨줘도 됨(이미 카드 데이터를 가지고 있으므로)
        if (id1 == LocalPlayer.ID)
        {
        ...
        //그러나 리모트는 현재 상태에서 카드가 더미일 것이기에, 데이터를 넣어줘야 함.
        //이전에 Broadcast_SomeoneSelectedCard2Delete 에서 2번째 인덱스의 카드를 버리는 에니메이션을 했을 것이므로,
        //GetCard(2)로 접근하여 값을 넣어주면 됨.
        foreach (var rp in RemotePlayerUIDrawers)
        {
            if (rp.Target.ID == id1)
            {

...
public class InGameServer_PUN : MonoBehaviourPunCallbacks, IOnEventCallback
{
    ...
    public class User
    {
        ...
        public int Id => Player.ActorNumber; //일단은
            ...
            case ePacketType_InGameServer.U2SResponse_SelectCard2Delete:
                {
                    ...
                    if (Rooms.TryGetValue(roomID, out var room))
                    {
                        ...
                        else
                        {
                            ...
                            room.Cards2Delete[id] = new Tuple<byte, byte>(cardType, cardValue); // 선택한 카드 저장
                            ...
                            foreach (var pl in room.Players)
                            {
                                ...
                                PacketWriter.WriteInt(id); // 선택한 플레이어 ID
                                SendPacket(pl);
                            }
                            ...
                            if (room.PlayerCounter_SelectedCard2Delete == 3)
                            {
                                foreach (var pl in room.Players)
                                {
                                    ...
                                    {
                                        PacketWriter.WriteInt(op.Key); // 선택한 플레이어 ID
                                        var card2Delete = op.Value;
                                        PacketWriter.WriteByte(card2Delete.Item1); // 카드 타입
                                        PacketWriter.WriteByte(card2Delete.Item2); // 카드 값
                                    }
                                    SendPacket(pl);
                                }
                    ...
                    if (Rooms.TryGetValue(roomID, out var room))
                    {
                        ...
                        if (room.GoPlayerCountInSpecialRule == 2)
                        {
                            ...
                            foreach (var pl in room.Players)
                            {
                                ...
                                {
                                    PacketWriter.WriteInt(sp); // 플레이어 ID
                            ...
                            if (room.PlayerCounter_SelectedCard2Delete == 2)
                            {
                                foreach (var pl in room.Players)
                                {
                                    ...
                                    foreach (var op in room.Cards2Delete)
                                    {
                                        ...
                                        PacketWriter.WriteByte(card2Remove.Item1); // 카드 타입
                                        PacketWriter.WriteByte(card2Remove.Item2); // 카드 값
                                    }
                                    SendPacket(pl);
                                }
                                room.Cards2Delete.Clear();
